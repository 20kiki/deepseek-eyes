...
| 模型 | 适用场景 | 精度 | 速度 |
| :--- | :--- | :--- | :--- |
| `qwen3-vl-plus`（默认）| **日常首选。** 照片、图表、小字、复杂场景。能识别艺术品作者、隐藏文字。 | ★★★ | ★★ |
| `qwen3.6-plus` | 上一代旗舰，vl-plus 不可用时备选。 | ★★ | ★★ |
| `qwen3.6-flash` | **只需快速看一眼时用。** 简单照片、日常随拍。 | ★ | ★★★ |

**实测对比**（复杂插画，均开启 `--high-res`）：
...
> **推荐：** 默认 `qwen3-vl-plus --high-res` 覆盖 90% 场景。需要快速预览时切到 `--model qwen3.6-flash --high-res`。
